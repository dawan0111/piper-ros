...
if [ "$EXPECTED_CAN_COUNT" -ne 1 ]; then
    ...
    for iface in $(ip -br link show type can | awk '{print $1}'); do
        BUS_INFO=$(ethtool -i "$iface" | awk '/bus-info/ {print $2}')

        if [ -z "$BUS_INFO" ]; then
            echo "Warning: No bus-info for $iface"
            continue
        fi

        echo "$iface is at bus $BUS_INFO"

        if [[ ${#USB_PORTS[@]} -eq 0 ]]; then
            USB_PORTS["$BUS_INFO"]="can_left:1000000"
        else
            USB_PORTS["$BUS_INFO"]="can_right:1000000"
        fi
    done
    # USB_PORTS["3-7:1.0"]="can_left:1000000"
    # USB_PORTS["3-1.3:1.0"]="can_right:1000000"
fi
